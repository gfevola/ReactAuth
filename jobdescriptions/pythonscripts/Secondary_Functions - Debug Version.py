# -*- coding: utf-8 -*-
"""
Created on Tue Jan 18 12:42:47 2022
...
def taglist_to_grams(alltagslist, allgramslist):
    ...
    for e in range(max(alltagslist['EntryNo'])):
        for f in range(max(alltagslist['GramNo'])):
            ...
            if len(filt) > 0:
                xval = addcategories(filt['TokenNum'],wordcount,True)
                xvals.append(xval)
...
def lda_grammodeling(uniqueStrings):
    patterns = uniqueStrings.loc[uniqueStrings['Category']!="Education",'Terms'].apply(lambda x: parse_term(x))
    ...
    xcomp = matrixsubtract(uniqueStrings[['X']])
    ycomp = matrixsubtract(uniqueStrings[['Y']])
    zcomp = matrixsubtract(uniqueStrings[['Z']])
    ...
    diffmelt = diffmelt.loc[(diffmelt['value'] < 20) & (diffmelt['UniqueGramNoFrom'] > diffmelt['UniqueGramNoTo']),]
...
def Texts_to_Seqs(texts):
    ...
    textdf['TermList'] = textdf.loc[:,'Text'].apply(lambda x: parse_term(x))
    textdf['POS']  = textdf.loc[:,'TermList'].apply(lambda x: nltk.pos_tag(x))
    textdf['POSFilt'] = textdf['POS'].apply(lambda x: filter_pos_tag(x))
    textdf['Seqs'] = textdf['POSFilt'].apply(lambda x: tokenizer.texts_to_sequences(x.loc[:,0]))
    textdf['SeqsUnlist'] =  textdf['Seqs'].apply(lambda x: intseq_unlist(x))
    numwords = len(tokenizer.word_counts)+1
    
    xvals = [addcategories(x,numwords,False).transpose() for x in textdf['SeqsUnlist']]  #changed addcategories since
